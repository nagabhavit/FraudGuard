...
Identical pattern to `gateway.middleware`, duplicated rather than shared --
each service owns its own small, independent copy rather than taking on a
cross-service dependency for ~60 lines.

Also records `fraudguard_http_request_duration_seconds` (ADR-0010) for
every request, labelled by the route's path *template*
(`request.scope["route"].path`), never the resolved URL, with an unmatched
route (a 404) labelled "unmatched" instead of the raw path.
"""
